...
if __name__ == "__main__":
	...
	f_net = FullyConnectedNet(state_dim, [64, 64])
	f_net.to(device)
	path = "/scratch/bdaw/kaiyan289/intentDICE/model/f_net_expert.pth"
	if os.path.exists(path):
		f_net.load_state_dict(torch.load(path))
	...
	for t in range(int(args.max_timesteps)):
		...
		reward = -f_net(torch.tensor(state, dtype=torch.float32).float().to(device)).cpu().detach().item() + 3
