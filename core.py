...
class Agent:
    def __init__(self, state_dim, action_dim, env, expert_buffer, args):
        ...
        if self.gail:
            self.discriminator = Discriminator(state_dim, action_dim, self.hidden_dims).to(self.device)
            self.d_optimizer = torch.optim.Adam(self.discriminator.parameters(), self.args.lr_d)
    ...
    def f_update(self):
        ...
        for _ in range(1, update_step):
            ...
            if self.state_action:
                loss_f = (torch.mean(self.f_net(self.expert_states, self.expert_actions)) - torch.mean(self.f_net(self.sample_states, self.sample_actions)))
    ...
    def pretrain(self):
        ...
        for epoch in range(2500):
            ...
            if self.using_icvf:
                random_states = self.phi_net(random_batch)
